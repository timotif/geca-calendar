...
class CalendarService():
	...
	def update_projects_parallel(self, projects: list, force_update=False) -> tuple[list, list]:
		...
		projects_fetched = []
		projects_updated = []
		# Collect all projects to update or fetch
		for p in projects:
			if force_update or not self.__is_project_up_to_date(p):
				projects_to_update.append(p)
			else:
				projects_fetched.append(self.db.get_by_id(p['id']).to_project_dto())
		# Use ThreadPoolExecutor to update projects in parallel
		with concurrent.futures.ThreadPoolExecutor() as executor:
			futures = [executor.submit(self.data_source.fetch_project, p) for p in projects_to_update]
			concurrent.futures.wait(futures)
		for p in projects_to_update:
			if not (project_dto := self.data_source.to_project_dto(p)):
				continue
			projects_updated.append(project_dto)
		self.db.save(projects_updated)
		logger.debug(f"Outdated projects: {[p.name for p in projects_updated]} ({len(projects_updated)} in total)")
